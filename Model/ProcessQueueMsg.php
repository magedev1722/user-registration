...
class ProcessQueueMsg extends \Magento\Framework\Model\AbstractModel
{
	...
	public function updateProductByEntity($params)
    {
       ...
       try {
            ...
            if ($product->save()) {
                $catalog_model = $objectManager->create('Aks\CustomCatalog\Model\CustomCatalog')->getCollection()->addFieldToFilter('product_id',$params['entity_id']);

                foreach ($catalog_model as $catalog) {
                    $catalog_id = $catalog->getId();
                }
                $model = $objectManager->create('Aks\CustomCatalog\Model\CustomCatalog')->load($catalog_id);
